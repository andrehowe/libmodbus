...
output:
    ...
    if (argc >= 4 ) {
        ...
        if (strcmp(argv[1], "read") == 0) {
            ...
            printf("Usage:\n  %s [read|write ip-address channel(0-7) high|low] - Modbus client for universal robot UR3E\n\n", argv[0]);
            exit(-42);
        ...
        if( modbus_channel < 0 || modbus_channel > 7 ) {
                    printf("Usage:\n  %s [read|write ip-address channel high|low] - Modbus client for universal robot UR3E\n\n", argv[0]);
                    exit(-42);
    ...
    if ( READ_SINGLE_COIL == modbus_cmd ) {
        ...
        rc = modbus_read_bits(ctx, modbus_channel/*UT_BITS_ADDRESS*/, 1/*UT_BITS_NB*/, tab_rp_bits);
        ASSERT_TRUE(rc == 1, "FAILED (nb points %d)\n", rc);
        //ASSERT_TRUE(tab_rp_bits[0] == ON, "FAILED (%0X != %0X)\n",
        //            tab_rp_bits[0], ON);
        ret_val = tab_rp_bits[0];

        
    } else if ( WRITE_SINGLE_COIL == modbus_cmd ) {

        rc = modbus_write_bit(ctx, modbus_channel/*UT_BITS_ADDRESS*/, modbus_bit);
        ASSERT_TRUE(rc == 1, "");

    }
    
    success = TRUE; //goto close;  
    ...
    return (success) ? ret_val : -1;
	

//    usleep(1000000);           

//    rc = modbus_read_bits(ctx, 16/*UT_BITS_ADDRESS*/, 1/*UT_BITS_NB*/, tab_rp_bits);
...
//    /* Single */
//    rc = modbus_write_bit(ctx, 16/*UT_BITS_ADDRESS*/, OFF);
...
//    printf("2/2 modbus_read_bits: ");
//    ASSERT_TRUE(rc == 1, "FAILED (nb points %d)\n", rc);
//    ASSERT_TRUE(tab_rp_bits[0] == OFF, "FAILED (%0X != %0X)\n",
//                tab_rp_bits[0], OFF);
